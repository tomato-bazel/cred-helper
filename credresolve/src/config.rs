...
fn config_path() -> Option<std::path::PathBuf> {
    ...
    crate::paths::config_dir().ok().map(|d| d.join("credentials.json"))
}
...
fn resolve_arm(arm: &Value) -> Option<ResolvedCred> {
    let header = arm
        .get("header")
    ...
    let secret = resolve_secret(arm.get("secret")?)?;
    Some(ResolvedCred {
        header,
...
/// `{"env":{…}}`, `{"file":{…}}`, or `{"awsEcr":{…}}`.
fn resolve_secret(secret: &Value) -> Option<String> {
    if let Some(env) = secret.get("env") {
        resolve_env(env)
    } else if let Some(file) = secret.get("file") {
        resolve_file(file)
    } else if let Some(ecr) = secret.get("awsEcr") {
        ecr::authorization_token(ecr.get("region").and_then(Value::as_str).unwrap_or(""))
    } else {
        None
    }
}
...
mod ecr {
    ...
    #[must_use]
    pub fn authorization_token(region: &str) -> Option<String> {
